...
def main():
    ...
    roi_c = {'x': 245, 'y': 210, 'w': 200, 'h': 80}
    
    umbral_altura_minima = 0.2  # cm
        ...
        while True:
            ...
            for cnt in contornos_c:
                if cv2.contourArea(cnt) > 500:
                    ...
                    color_display = (0, 255, 0) #detectado

            #comprobamos si existe aún area en el roi_v
            ...
            for cnt in contornos_v:
                if cv2.contourArea(cnt) > 500:
                    ...
                    # calculo velocidad 
                    centro_y = vy + vh // 2
                    tiempo_actual = frames.get_timestamp() / 1000.0 # Convertir ms a segundos
                    
                    if ultimo_y is not None and ultimo_tiempo is not None:
                        dt = tiempo_actual - ultimo_tiempo
                        if dt > 0:
                            dy = abs(centro_y - ultimo_y)
                            # Velocidad en px/s
                            v_px_s = dy / dt
                            
                            # Velocidad en m/s 
                            cm_por_px = altura_retenida / vh if vh > 0 else 0
                    ...
                    info_vel = f"{v_px_s:.1f} px/s | {v_m_s:.2f} m/s"
                    cv2.putText(color_image, info_vel, (vx, vy + vh + 20), 
            ...
            #cv2.putText(color_image, "a ver", (roi_v['x']+5, roi_v['y']+20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 0), 1)
            #cv2.putText(color_image, "a calculo", (roi_c['x']+5, roi_c['y']+20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)

            cv2.imshow('Alturas', color_image)
